...
function moverColumnaHiloSeguimiento(core, acciones, { hiloId, columnaSeguimientoId }) {
  ...
  if (!core.hilos.find((h) => h.id === hiloId)) throw new Error('No se encontró el hilo.');
  return {
    core: { ...core, hilos: core.hilos.map((h) => (h.id === hiloId ? { ...h, columnaSeguimientoId: columnaSeguimientoId || null } : h)) },
    acciones,
    resultado: { hiloId },
  };
}
...
// seguimientos/tareas, y tareas/subtareas con fecha propia) a una fecha nueva — mismo
// criterio que el botón "Reprogramar vencidas" del Resumen de hoy: no propone una fecha
// ajustada por día hábil/sobrecargado (eso es SelectorProgramacion, para un solo registro a
// la vez), es un movimiento masivo explícito con la fecha tal cual se pida.
// ---------------------------------------------------------------------------
function reprogramarVencidas(core, acciones, { fecha }) {
  ...
  const hilosConFecha = new Set(
    core.hilos.filter((h) => h.tipo === 'tarea' && h.estado === 'Activo' && h.fecha && h.fecha < hoy).map((h) => h.id)
  );
  ...
  const nuevosHilos = hilosConFecha.size === 0 && subtareasPorHilo.size === 0 ? core.hilos : core.hilos.map((h) => {
    let hh = h;
    if (hilosConFecha.has(h.id)) hh = { ...hh, fecha, avisoEnviado: false, avisoVistoEnApp: false };
    const subIds = subtareasPorHilo.get(h.id);
    if (subIds) hh = { ...hh, subtareas: (hh.subtareas || []).map((s) => (subIds.has(s.id) ? { ...s, fecha, avisoEnviado: false, avisoVistoEnApp: false } : s)) };
    return hh;
  });
  ...
  return {
    ...
    resultado: { reprogramadas: idsAcciones.size + hilosConFecha.size + totalSubtareas },
  };
}
