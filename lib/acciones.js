...
// Arma los vínculos genéricos Hilo<->Persona/Empresa/Obra al crear un hilo (de seguimiento o
// tarea) — personaPrincipal decide si el vínculo con la persona queda marcado como principal
// (seguimiento, donde la persona es el foco del hilo) o no (tarea, donde es solo una referencia).
function vinculosAlCrearHilo(hiloId, { personaId, empresaIds = [], obraIds = [] }, personaPrincipal) {
  const nuevosVinculos = [];
  if (personaId) nuevosVinculos.push(vinc('Persona', personaId, 'Hilo', hiloId, null, personaPrincipal, todayISO()));
...
function crearHiloSeguimiento(core, acciones, { titulo, personaId = null, empresaIds = [], obraIds = [] }) {
  ...
  const nuevosVinculos = vinculosAlCrearHilo(hiloId, { personaId, empresaIds, obraIds }, true);
  return {
...
function cerrarHilo(core, acciones, { hiloId, notaCierre = '' }) {
  ...
  const hilo = core.hilos.find((h) => h.id === hiloId);
  if (!hilo) throw new Error('No se encontró el hilo.');
  ...
  const nuevasAcciones = acciones.filter((a) => !(a.hiloId === hiloId && a.estado === 'Pendiente'));
  const hilos = core.hilos.map((h) => {
    if (h.id !== hiloId) return h;
    const next = { ...h, estado: 'Cerrado', notaCierre };
    if (h.tipo === 'tarea' && (h.subtareas || []).some((s) => !s.hecha)) {
      next.subtareas = (h.subtareas || []).map((s) => (s.hecha ? s : { ...s, hecha: true }));
    }
    return next;
  });
  return {
...
function accionPendienteDeTarea(hiloId, { fecha, hora, aviso, recurrente, repiteCadaN, repiteUnidad }, acciones) {
  const numero = Math.max(0, ...acciones.map((a) => a.numero || 0)) + 1;
  return {
    id: uid('A'), hiloId, tipoAccionId: '', estado: 'Pendiente',
    fechaRealizada: '', fechaProgramada: fecha, horaProgramada: hora || '', prioridad: 'Media',
    ...
    fechaCreacion: todayISO(), secuencia: Date.now(), aviso: aviso || null, avisoEnviado: false, avisoVistoEnApp: false,
  };
}
...
function crearTarea(core, acciones, { titulo, fecha = '', hora = '', aviso = null, columnaTareaId = null, personaId = null, empresaIds = [], obraIds = [], notaInicial = '', subtareasIniciales = [], recurrente = false, repiteCadaN = null, repiteUnidad = null }) {
  ...
  const nuevaTarea = {
    id: hiloId, titulo: titulo.trim(), notas,
  ...
  const nuevosVinculos = vinculosAlCrearHilo(hiloId, { personaId, empresaIds, obraIds }, false);
  // La fecha de una tarea vive en una acción pendiente, mismo mecanismo que un seguimiento.
  const nuevasAcciones = fecha
    ? [accionPendienteDeTarea(hiloId, { fecha, hora, aviso: aviso || { ...AVISO_DEFAULT }, recurrente, repiteCadaN, repiteUnidad }, acciones), ...acciones]
    : acciones;
  return {
...
// ---------------------------------------------------------------------------
// 5b. Editar título/fecha/hora de una tarea, y moverla de columna en el Kanban de Tareas. La
// fecha vive en la acción pendiente de la tarea: se edita si ya existe, se crea si no, o se
// elimina si se pide "" para sacarle la fecha.
// ---------------------------------------------------------------------------
function editarTarea(core, acciones, { hiloId, titulo, fecha, hora, aviso, recurrente, repiteCadaN, repiteUnidad }) {
  ...
  const nuevoCore = { ...core, hilos: core.hilos.map((h) => (h.id === hiloId ? { ...h, ...patch } : h)) };

  let nuevasAcciones = acciones;
  if (fecha !== undefined || hora !== undefined || aviso !== undefined || recurrente !== undefined) {
    const pendiente = acciones.find((a) => a.hiloId === hiloId && a.estado === 'Pendiente');
    if (fecha === '') {
      if (pendiente) nuevasAcciones = acciones.filter((a) => a.id !== pendiente.id);
    } else if (pendiente) {
      const patchAccion = {};
      if (fecha !== undefined) patchAccion.fechaProgramada = fecha;
      if (hora !== undefined) patchAccion.horaProgramada = hora;
      if (aviso !== undefined) patchAccion.aviso = aviso;
      if (recurrente !== undefined) {
        patchAccion.recurrente = !!recurrente;
        patchAccion.repiteCadaN = recurrente ? repiteCadaN : null;
        patchAccion.repiteUnidad = recurrente ? repiteUnidad : null;
      }
      nuevasAcciones = acciones.map((a) => (a.id === pendiente.id ? { ...a, ...patchAccion } : a));
    } else if (fecha) {
      nuevasAcciones = [accionPendienteDeTarea(hiloId, { fecha, hora: hora || '', aviso: aviso || null, recurrente, repiteCadaN, repiteUnidad }, acciones), ...acciones];
    }
  }

  return { core: nuevoCore, acciones: nuevasAcciones, resultado: { hiloId } };
}

// Mueve un hilo (de seguimiento o de tarea, según "campo") a otra columna de su Kanban —
// misma operación para los dos tipos, solo cambia qué campo del hilo se toca.
function moverColumna(core, acciones, { hiloId, campo, valor }) {
  if (!hiloId) throw new Error('Falta el hilo a mover.');
  if (!core.hilos.find((h) => h.id === hiloId)) throw new Error('No se encontró el hilo.');
  return {
    core: { ...core, hilos: core.hilos.map((h) => (h.id === hiloId ? { ...h, [campo]: valor || null } : h)) },
    acciones,
    resultado: { hiloId },
  };
}
...
function moverColumnaTarea(core, acciones, { hiloId, columnaTareaId }) {
  return moverColumna(core, acciones, { hiloId, campo: 'columnaTareaId', valor: columnaTareaId });
}

function moverColumnaHiloSeguimiento(core, acciones, { hiloId, columnaSeguimientoId }) {
  return moverColumna(core, acciones, { hiloId, campo: 'columnaSeguimientoId', valor: columnaSeguimientoId });
}
...
// ---------------------------------------------------------------------------
// 9. Reprogramar en bloque todas las actividades vencidas (acciones pendientes de
// seguimientos/tareas — la fecha propia de una tarea vive ahí, mismo mecanismo que un
// seguimiento — y subtareas con fecha propia) a una fecha nueva — mismo criterio que el botón
// "Reprogramar" del Resumen de hoy: no propone una fecha ajustada por día hábil/sobrecargado
// (eso es SelectorProgramacion, para un solo registro a la vez), es un movimiento masivo
// explícito con la fecha tal cual se pida.
// ---------------------------------------------------------------------------
function reprogramarVencidas(core, acciones, { fecha }) {
  if (!fecha) throw new Error('Falta la fecha para reprogramar.');
  const hoy = todayISO();

  const idsAcciones = new Set(
    acciones.filter((a) => a.estado === 'Pendiente' && a.fechaProgramada && a.fechaProgramada < hoy).map((a) => a.id)
  );
  ...
  for (const h of core.hilos) {
    ...
    for (const s of h.subtareas || []) {
      if (s.hecha || !s.fecha || s.fecha >= hoy) continue;
      if (!subtareasPorHilo.has(h.id)) subtareasPorHilo.set(h.id, new Set());
      subtareasPorHilo.get(h.id).add(s.id);
    }
  }

  const nuevasAcciones = idsAcciones.size === 0 ? acciones : acciones.map((a) =>
    idsAcciones.has(a.id) ? { ...a, fechaProgramada: fecha, avisoEnviado: false, avisoVistoEnApp: false } : a
  );
  const nuevosHilos = subtareasPorHilo.size === 0 ? core.hilos : core.hilos.map((h) => {
    const subIds = subtareasPorHilo.get(h.id);
    if (!subIds) return h;
    return { ...h, subtareas: (h.subtareas || []).map((s) => (subIds.has(s.id) ? { ...s, fecha, avisoEnviado: false, avisoVistoEnApp: false } : s)) };
  });

  const totalSubtareas = [...subtareasPorHilo.values()].reduce((n, set) => n + set.size, 0);
  return {
    core: { ...core, hilos: nuevosHilos },
    acciones: nuevasAcciones,
    resultado: { reprogramadas: idsAcciones.size + totalSubtareas },
  };
}
...
export const CATALOGO_ACCIONES = [
  ...
  {
    nombre: 'moverColumnaHiloSeguimiento',
    descripcion: 'Mueve un hilo de seguimiento a otra columna del Kanban de Seguimientos.',
    parametros: {
      type: 'object',
      properties: { hiloId: { type: 'string' }, columnaSeguimientoId: { type: 'string', description: 'Id de la columna destino, o "" para "Sin columna".' } },
      required: ['hiloId'],
    },
    ejecutar: moverColumnaHiloSeguimiento,
  },
  {
    nombre: 'cerrarHilo',
  ...
  {
    nombre: 'reprogramarVencidas',
    descripcion: 'Reprograma en bloque todas las actividades vencidas (acciones pendientes de seguimientos y tareas, tareas y subtareas con fecha propia) a una fecha nueva. No propone una fecha ajustada por día hábil o sobrecargado — usa la fecha tal cual se pida, para todas a la vez.',
    parametros: {
      type: 'object',
      properties: { fecha: { type: 'string', description: 'Fecha ISO (AAAA-MM-DD) a la que se reprograman todas las actividades vencidas.' } },
      required: ['fecha'],
    },
    ejecutar: reprogramarVencidas,
  },
];
