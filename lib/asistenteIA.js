...
const FORMAS_PARAMETROS = {
  ...
  crearTarea: {
    type: 'object',
    properties: {
      titulo: { type: 'string' }, fecha: { type: 'string' }, hora: { type: 'string' }, aviso: AVISO_SCHEMA,
      personaNombre: { type: 'string' }, empresaNombres: { type: 'array', items: { type: 'string' } }, obraNombres: { type: 'array', items: { type: 'string' } },
  ...
  editarTarea: {
    type: 'object',
    properties: {
      hiloNombre: { type: 'string' }, titulo: { type: 'string' }, fecha: { type: 'string' }, hora: { type: 'string' }, aviso: AVISO_SCHEMA,
      recurrente: { type: 'boolean' }, repiteCadaN: { type: 'number' }, repiteUnidad: { type: 'string', enum: ['dias', 'semanas', 'meses'] },
    },
    required: ['hiloNombre'],
  },
  ...
  moverColumnaHiloSeguimiento: { type: 'object', properties: { hiloNombre: { type: 'string' }, columnaNombre: { type: 'string' } }, required: ['hiloNombre', 'columnaNombre'] },
  reprogramarVencidas: { type: 'object', properties: { fecha: { type: 'string' } }, required: ['fecha'] },
  avanzarHilo: {
    type: 'object',
    properties: {
...
15. crearTarea: { titulo*, fecha?, hora?, aviso?: { activo: boolean, cantidad: number, unidad: "minutos"|"horas"|"dias" }, personaNombre?, empresaNombres?: string[], obraNombres?: string[], notaInicial?, subtareasIniciales?: { texto*, fecha?, hora?, nota? }[], recurrente?, repiteCadaN?, repiteUnidad?: "dias"|"semanas"|"meses" }
16. editarTarea: { hiloNombre*, titulo?, fecha?, hora?, aviso?, recurrente?, repiteCadaN?, repiteUnidad?: "dias"|"semanas"|"meses" }
...
33. moverColumnaHiloSeguimiento: { hiloNombre*, columnaNombre* (nombre real de la columna del Kanban de Seguimientos, o "Sin columna") }
34. reprogramarVencidas: { fecha* } (reprograma TODAS las acciones pendientes, tareas y subtareas vencidas a esa fecha, de una sola vez — no ajusta por día hábil, usa la fecha tal cual)
...
const RESOLVEDORES = {
  ...
  crearTarea(core, acciones, p) {
    ...
    const obras = resolverVariasEntidades(core, 'Obra', p.obraNombres || []);
    if (!obras.ok) return obras;
    const subtareasIniciales = (p.subtareasIniciales || []).map((s) => ({ texto: s.texto || '', fecha: s.fecha || '', hora: s.hora || '', nota: s.nota || '' }));
    return {
      ok: true,
      parametros: {
        titulo: p.titulo, fecha: p.fecha || '', hora: p.hora || '', aviso: p.aviso || null, personaId, empresaIds: empresas.ids, obraIds: obras.ids,
        notaInicial: p.notaInicial || '', subtareasIniciales,
        recurrente: !!p.recurrente, repiteCadaN: p.repiteCadaN || null, repiteUnidad: p.repiteUnidad || null,
      },
    };
  },
  ...
  editarTarea(core, acciones, p) {
    ...
    if (p.recurrente !== undefined) {
      parametros.recurrente = p.recurrente;
      parametros.repiteCadaN = p.repiteCadaN || null;
      parametros.repiteUnidad = p.repiteUnidad || null;
    }
    return { ok: true, parametros };
  },
  ...
  moverColumnaHiloSeguimiento(core, acciones, p) {
    const hilo = resolverUnaEntidad(core, 'Hilo', p.hiloNombre);
    if (!hilo.ok) return hilo;
    if (!p.columnaNombre || !p.columnaNombre.trim()) return { ok: false, motivo: 'falta_dato', detalle: 'Falta la columna destino.' };
    const sinColumna = (core.parametros?.nombreSinColumnaSeguimientos || 'Sin columna').trim().toLowerCase();
    if (p.columnaNombre.trim().toLowerCase() === sinColumna) {
      return { ok: true, parametros: { hiloId: hilo.id, columnaSeguimientoId: '' } };
    }
    const col = resolverPorNombreEnLista(core.kanbanColumnas || [], 'nombre', p.columnaNombre, 'la columna');
    if (!col.ok) return col;
    return { ok: true, parametros: { hiloId: hilo.id, columnaSeguimientoId: col.id } };
  },
  ...
  reprogramarVencidas(core, acciones, p) {
    if (!p.fecha) return { ok: false, motivo: 'falta_dato', detalle: 'Falta la fecha a la que reprogramar.' };
    return { ok: true, parametros: { fecha: p.fecha } };
  },
};
...
export function resumenLegible(core, acciones, accion, p) {
  switch (accion) {
    ...
    case 'crearTarea': {
      ...
      const vinculos = [...(p.personaId ? [nombrePersona(core, p.personaId)] : []), ...(p.empresaIds || []).map((id) => nombreEmpresa(core, id)), ...(p.obraIds || []).map((id) => nombreObra(core, id))];
      if (vinculos.length) texto += `, vinculada a ${vinculos.join(', ')}`;
      if (p.notaInicial) texto += `, con la nota "${p.notaInicial}"`;
      for (const s of p.subtareasIniciales || []) {
        texto += `, con la subtarea "${s.texto}"`;
        if (s.fecha) texto += ` para el ${fmtFechaHora(s.fecha, s.hora)}`;
        if (s.nota) texto += ` (nota: "${s.nota}")`;
      }
      if (p.recurrente) texto += `, repetitiva cada ${p.repiteCadaN} ${p.repiteUnidad}`;
      return texto + '.';
    }
    case 'editarTarea': {
      const cambios = textoPatch(p, { titulo: 'título', fecha: 'fecha', hora: 'hora' });
      let texto = `Editar la tarea "${nombreHilo(core, p.hiloId)}"${cambios ? ` — ${cambios}` : ''}.`;
      if (p.recurrente !== undefined) texto += p.recurrente ? ` Repetitiva cada ${p.repiteCadaN} ${p.repiteUnidad}.` : ' Deja de ser repetitiva.';
      return texto;
    }
    ...
    case 'moverColumnaHiloSeguimiento': {
      const columna = p.columnaSeguimientoId ? (core.kanbanColumnas || []).find((c) => c.id === p.columnaSeguimientoId)?.nombre || '(columna no encontrada)' : (core.parametros?.nombreSinColumnaSeguimientos || 'Sin columna');
      return `Mover el hilo "${nombreHilo(core, p.hiloId)}" a la columna "${columna}".`;
    }
    case 'avanzarHilo': {
    ...
    case 'reprogramarVencidas': {
      const hoy = new Date().toISOString().slice(0, 10);
      const nAcciones = acciones.filter((a) => a.estado === 'Pendiente' && a.fechaProgramada && a.fechaProgramada < hoy).length;
      const nTareas = core.hilos.filter((h) => h.tipo === 'tarea' && h.estado === 'Activo' && h.fecha && h.fecha < hoy).length;
      const nSubtareas = core.hilos.reduce((n, h) => n + (h.tipo === 'tarea' && h.estado === 'Activo' ? (h.subtareas || []).filter((s) => !s.hecha && s.fecha && s.fecha < hoy).length : 0), 0);
      const total = nAcciones + nTareas + nSubtareas;
      return `Reprogramar ${total} actividad${total === 1 ? '' : 'es'} vencida${total === 1 ? '' : 's'} para el ${fmtFecha(p.fecha)}.`;
    }
    default:
