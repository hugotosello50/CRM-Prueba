...
15. crearTarea: { titulo*, fecha?, hora?, aviso?: { activo: boolean, cantidad: number, unidad: "minutos"|"horas"|"dias" }, personaNombre?, empresaNombres?: string[], obraNombres?: string[] }
...
const RESOLVEDORES = {
  ...
  crearTarea(core, acciones, p) {
    ...
    const obras = resolverVariasEntidades(core, 'Obra', p.obraNombres || []);
    if (!obras.ok) return obras;
    return { ok: true, parametros: { titulo: p.titulo, fecha: p.fecha || '', hora: p.hora || '', aviso: p.aviso || null, personaId, empresaIds: empresas.ids, obraIds: obras.ids } };
  },
