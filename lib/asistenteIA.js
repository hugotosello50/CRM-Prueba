...
27. vincularEntidades: { origenes*: {tipo: "Persona"|"Empresa"|"Obra", nombre}[], destinos*: (mismo formato), nota? }
28. asignarEtiqueta: { entidadTipo* ("Persona"|"Empresa"|"Obra"), entidadNombres*: string[], nombreEtiqueta*, categoriaNombre? }
...
export function resumenLegible(core, acciones, accion, p) {
  switch (accion) {
    ...
    case 'eliminarPersona':
      return `Eliminar a ${nombrePersona(core, p.personaId)} (y sus vínculos con otras entidades).`;
    case 'crearEmpresa': {
      const detalles = [];
    ...
    case 'eliminarEmpresa':
      return `Eliminar la empresa ${nombreEmpresa(core, p.empresaId)} (y sus vínculos con otras entidades).`;
    ...
    case 'eliminarObra':
      return `Eliminar la obra ${nombreObra(core, p.obraId)} (y sus vínculos con otras entidades).`;
